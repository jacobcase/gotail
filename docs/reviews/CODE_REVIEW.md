...
`internal/bufpool/bufpool.go` — verified via grep, no callers anywhere in the tree. The V2_PLAN listed buffer reuse as a goal, the LineReader did the work in-place (`linereader.go:111–119`), and this package was left behind. Either delete or wire it in for the rotation case (`watch/linereader.go:226 switchToFile` allocates fresh state but reuses `l.buf`).
